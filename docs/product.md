...
## Process
Hermes reviews -> saves detailed GLM 5.2 cloud prompt -> OpenCode implements one step -> Hermes independently verifies -> user approves next step. Initial request authorizes Step01 foundation only, not the full roadmap. Later prompts are written just in time against actual implementation.
