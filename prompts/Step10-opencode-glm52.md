...
This entire file is a ready-to-run prompt, not merely a feature outline. Saving it does not authorize execution. `steps/Step10.md` is canonical; `prompts/Step10-opencode-glm52.md` is its identical execution copy. Keep both synchronized after any preflight amendment.

## Prerequisites / hard gate
Step09 approved. Review financial rules carefully before execution.
...
1. Read exact-cent functions and account refresh/transfer locking protocol. Implement backend-only payment/receipt/reversal operations reused by later UI and reserve settlements. Use one transaction client for every write and deterministic lock ordering across operation, account, obligation and month rows.
2. Verify session owner and owner-aware relationships, open-period policy, account eligibility, requested amount/mode/date and current remaining under lock. UPDATE_ACCOUNT changes B; ALREADY_REFLECTED does not. Never infer mode. Reject zero/negative/overpayment.
3. Claim owner+operation idempotency key atomically; identical retry returns original safe result, changed payload with same key rejects. Locking plus constraints must prevent two concurrent payments exceeding remaining or double debiting. Handle transaction failures without partial audit/movement or phantom success.
4. Insert immutable settlement, account movement when required, version update and audit atomically. Receipts mirror payment with addition. Original planned amount remains unchanged. Reserved targets reduce their own outstanding amount and never produce another ordinary liability.
5. Reversal uses compensating entries linked to original. Guard double reversal and reverse idempotently. If account was manually refreshed after original settlement, require explicit reconciliation/balance-effect decision, not a blind credit/debit. Keep business date versus recorded-at provenance distinct.
6. Return minimal typed results and actionable safe conflicts. No secrets/full rows in errors. Revalidate affected data only after commit. Document lock order, retry protocol and post-refresh reversal behavior in docs/settlements.md.
7. Prove real service tests: 8000 -> 4000 -> 0; third payment rejects; simultaneous 6000+6000 cannot both settle 8000; duplicate request debits once; changed-payload key rejects; injected rollback; new/already-reflected payment and income; reserve target; cross-owner refs; concurrent refresh; reversal before/after refresh. Test production helpers, not duplicated SQL formulas.
...
Write `docs/reviews/Step10-opencode.md` with scope, files, schema/API decisions, real commands/results, failures/blockers, synthetic fixture cleanup and remaining risks. This is the implementer's self-report, not Hermes approval. For completed Step01 preserve the historical self-report and write a separately named verification addendum instead of rewriting history.
Hermes independently inspects diff, reruns relevant checks/browser/DB probes and records `docs/reviews/Step10.md`. Do not mark the step complete yourself before this gate. Update roadmap statuses only from verified evidence. STOP after this step; do not run the next prompt. Return a concise handoff and wait for approval.

## Launch (operator reference; do not recursively launch yourself)
```bash
cd /home/leandrosdim777/projects/mySavings
opencode run --agent build --model ollama-cloud/glm-5.2 "$(< prompts/Step10-opencode-glm52.md)"
```
