...
This entire file is a ready-to-run prompt, not merely a feature outline. Saving it does not authorize execution. `steps/Step08.md` is canonical; `prompts/Step08-opencode-glm52.md` is its identical execution copy. Keep both synchronized after any preflight amendment.
...
1. Implement monthly-plan service with unique owner/year-month, documented Europe/Athens boundary behavior and exact-cent protected savings target. Target 0 is explicit, missing target is incomplete. Never treat it as a bank movement or monthly deposit.
2. CRUD recurring expense/income definitions with names, planned amounts, type/category where already modeled, active flag and monthly generation rule. Do not create a full recurrence engine: a weekly spending allowance may be entered as a monthly budget, not silently multiplied by an assumed four weeks.
3. Generate selected/current month entries from templates once using database unique keys and transaction-safe operation idempotency. Editing a template affects future generation only; keep generated instance amount/history stable. Templates themselves do not enter E or I.
4. Add mobile plan/target/template screens with confirmation, empty/error/loading states, parser validation and persisted values. Do not implement full next-month carryover workflow before Step14.
5. Month-target changes update displayed target/derived screens when they exist. Reject edits to closed months once lifecycle is in force. Authorization checks apply to plan/template and all referenced IDs.
6. Test repeated/concurrent generation, zero/missing/invalid target, month boundaries including year rollover, template edit/deactivate preserving existing instances, two-user isolation and real browser form persistence.
...
Write `docs/reviews/Step08-opencode.md` with scope, files, schema/API decisions, real commands/results, failures/blockers, synthetic fixture cleanup and remaining risks. This is the implementer's self-report, not Hermes approval. For completed Step01 preserve the historical self-report and write a separately named verification addendum instead of rewriting history.
Hermes independently inspects diff, reruns relevant checks/browser/DB probes and records `docs/reviews/Step08.md`. Do not mark the step complete yourself before this gate. Update roadmap statuses only from verified evidence. STOP after this step; do not run the next prompt. Return a concise handoff and wait for approval.

## Launch (operator reference; do not recursively launch yourself)
```bash
cd /home/leandrosdim777/projects/mySavings
opencode run --agent build --model ollama-cloud/glm-5.2 "$(< prompts/Step08-opencode-glm52.md)"
```
