...
This entire file is a ready-to-run prompt, not merely a feature outline. Saving it does not authorize execution. `steps/Step09.md` is canonical; `prompts/Step09-opencode-glm52.md` is its identical execution copy. Keep both synchronized after any preflight amendment.
...
1. Build ordinary expense-plan and expected-income create/list/detail/update/archive on the existing schema. Use selected owner/month, exact cents, optional valid due/business dates and existing categories only. Do not invent a new accounting ledger or bank integration.
2. Preserve original planned amount and display paid/remaining derived from non-reversed settlements, even though payment writes arrive at Step10. Define revision rules: no lowering plan below already-settled amount; audit revisions; no deletion of paid history. Empty month is not fabricated sample data.
3. Support fixed costs and variable monthly envelopes such as gas. Users can spend envelopes in multiple visits later; no automatic week-count multiplication. Keep actual spending distinct from planned allowance.
4. Expected income shows planned/received/pending; manual bank refresh must not imply an automatic receipt. Income cannot be simultaneously counted as already in B and still pending I without a visible reconciliation task.
5. Implement mobile activity lists/detail forms/filter by selected month and type. Keep reserve items out of ordinary expense aggregates; no duplicated reserve creation interface. No enabled payment/receipt mutation controls before Step11.
6. Test owner/month filtering, invalid dates/amounts, changing month while form open, forbidden closed-period edits where applicable, paid-history preservation and direct cross-owner IDs. Browser-create/update/archive test expense/income and verify database state and empty/error views.
...
Write `docs/reviews/Step09-opencode.md` with scope, files, schema/API decisions, real commands/results, failures/blockers, synthetic fixture cleanup and remaining risks. This is the implementer's self-report, not Hermes approval. For completed Step01 preserve the historical self-report and write a separately named verification addendum instead of rewriting history.
Hermes independently inspects diff, reruns relevant checks/browser/DB probes and records `docs/reviews/Step09.md`. Do not mark the step complete yourself before this gate. Update roadmap statuses only from verified evidence. STOP after this step; do not run the next prompt. Return a concise handoff and wait for approval.

## Launch (operator reference; do not recursively launch yourself)
```bash
cd /home/leandrosdim777/projects/mySavings
opencode run --agent build --model ollama-cloud/glm-5.2 "$(< prompts/Step09-opencode-glm52.md)"
```
