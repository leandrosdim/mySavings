...
This entire file is a ready-to-run prompt, not merely a feature outline. Saving it does not authorize execution. `steps/Step13.md` is canonical; `prompts/Step13-opencode-glm52.md` is its identical execution copy. Keep both synchronized after any preflight amendment.
...
1. Inspect route layout before creating overview: avoid two pages resolving to /. Replace/move the foundation route intentionally, preserve login and use session-protected real data. Never expose private server-rendered finance through shared caching.
2. Create one owner-scoped dashboard query/service feeding Step02 pure functions. Aggregate settlement totals without fan-out join multiplication. Distinguish ordinary unpaid E from outstanding reserved R, include same-identity carried obligations only once, pending income I and continuous account B.
3. Primary UI prioritizes protected savings target, target shortfall and projected versus cash-backed free-to-spend. Explain pending income dependence and stale/unentered balances. Do not show confident zero or positive available money before setup is complete.
4. Drilldown explains B/I/E/R/S with links to implemented data screens. Reserve due date does not remove protection now. Negative results remain visible, no celebratory state on a projected deficit. Labels must not call projected balances actual results.
5. Make current/open-month semantics explicit: historical months are not reconstructed using today's live B; render unsupported closed-month comparison unavailable until Step15 snapshots exist. Do not fabricate actual spending from balance adjustments.
6. Ensure payments/receipts/refresh/target edits/reserve changes invalidate overview and component summaries immediately. Validate A/B ownership and no-store semantics.
7. Build an independently computed synthetic fixture using checked cents, compare persisted inputs and displayed output to formula. Browser-test partial payment, already-reflected reconciliation, reserve settlement, stale data, negative/missing cases and phone layouts. No actual workbook figures in fixtures.
...
Write `docs/reviews/Step13-opencode.md` with scope, files, schema/API decisions, real commands/results, failures/blockers, synthetic fixture cleanup and remaining risks. This is the implementer's self-report, not Hermes approval. For completed Step01 preserve the historical self-report and write a separately named verification addendum instead of rewriting history.
Hermes independently inspects diff, reruns relevant checks/browser/DB probes and records `docs/reviews/Step13.md`. Do not mark the step complete yourself before this gate. Update roadmap statuses only from verified evidence. STOP after this step; do not run the next prompt. Return a concise handoff and wait for approval.

## Launch (operator reference; do not recursively launch yourself)
```bash
cd /home/leandrosdim777/projects/mySavings
opencode run --agent build --model ollama-cloud/glm-5.2 "$(< prompts/Step13-opencode-glm52.md)"
```
